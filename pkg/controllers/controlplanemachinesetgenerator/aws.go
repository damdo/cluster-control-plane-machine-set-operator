...
func buildControlPlaneMachineSetAWSMachineSpec(machines []machinev1beta1.Machine) (*machinev1beta1builder.MachineSpecApplyConfiguration, error) {
	...
	return &machinev1beta1builder.MachineSpecApplyConfiguration{
		ProviderSpec: &machinev1beta1builder.ProviderSpecApplyConfiguration{Value: rawProviderSpecValue},
	}, nil
}

// extractAWSProviderSpec extracts an AWS Provider Spec from a generic machine Provider Spec.
func extractAWSProviderSpec(providerSpec machinev1beta1.ProviderSpec) (*machinev1beta1.AWSMachineProviderConfig, error) {
	ps, err := providerSpecFromRawExtension(configv1.AWSPlatformType, providerSpec.Value)
	if err != nil {
		return nil, fmt.Errorf("failed to decode machine providerSpec: %w", err)
	}

	awsPs, ok := ps.(machinev1beta1.AWSMachineProviderConfig)
	if !ok {
