...
var (
	...
	// errFailedProviderSpecTypeAssertion defines an error for a failed providerSpec type assertion.
	errFailedProviderSpecTypeAssertion = errors.New("failed providerSpec type assertion")
	// errEmptyProviderSpec defines an error for an empty providerSpec.
	errEmptyProviderSpec = errors.New("empty providerSpec")
)
...
func (r *ControlPlaneMachineSetGeneratorReconciler) reconcile(ctx context.Context, logger logr.Logger,
	...
	if diff, err := compareControlPlaneMachineSets(infrastructure.Spec.PlatformSpec.Type, cpms, generatedCPMS); err != nil {
