	...
	Context("when a Control Plane Machine Set doesn't exist", func() {
		...
		Context("with 3 existing control plane machines", func() {
			...
			It("should create the ControlPlaneMachineSet with the provider spec matching the youngest machine provider spec", func() {
				...
				cpmsProviderSpec, err := extractAWSProviderSpec(cpms.Spec.Template.OpenShiftMachineV1Beta1Machine.Spec.ProviderSpec)
				Expect(err).To(BeNil())

				machineProviderSpec, err := extractAWSProviderSpec(machine2.Spec.ProviderSpec)
				Expect(err).To(BeNil())

				// Remove from the machine Provider Spec the fields that won't be
				// present on the ControlPlaneMachineSet Provider Spec.
				machineProviderSpec.Subnet = machinev1beta1.AWSResourceReference{}
				machineProviderSpec.Placement.AvailabilityZone = ""

				Expect(cpmsProviderSpec).To(Equal(machineProviderSpec))
			})
		...
		Context("with state Inactive and outdated", func() {
			...
			It("should recreate ControlPlaneMachineSet with the provider spec matching the youngest machine provider spec", func() {
				...
				Eventually(komega.Object(cpms)).Should(
					HaveField("Spec.Template.OpenShiftMachineV1Beta1Machine.Spec.ProviderSpec", WithTransform(extractAWSProviderSpec, Equal(machineProviderSpec))),
